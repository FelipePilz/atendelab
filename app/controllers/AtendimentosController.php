...
class AtendimentosController
{
    ...
    public function buscar(): void
    {
        ...
        $stmt = $this->pdo->prepare('SELECT a.*, p.nome AS pessoa_nome,
                    t.nome AS tipo_nome,
                    u.nome AS responsavel_nome
                FROM atendimentos a 
                INNER JOIN pessoas p ON p.id = a.pessoa_id
        ...
        if (!$atendimento) {
            $this->json(['erro' => 'Atendimento não encontrada'], 404);
            return;
        }
    ...
    public function criar(): void
    {
        ...
        try {
            $stmt = $this->pdo->prepare('INSERT INTO atendimentos (pessoa_id, tipo_atendimento_id, usuario_id, descricao, status, data_atendimento, horario_atendimento) VALUES (:pessoa_id, :tipo_id, :usuario_id, :descricao, :status, :data_atendimento, :horario_atendimento)');
            $stmt->execute([
                'pessoa_id' => $pessoaId,
                'tipo_id' => $tipoId,
            ...
            $this->json(['mensagem' => 'Atendimento registrado com suceso.'], 201);
        } catch (PDOException $e) {
            $this->json(['erro' => 'Não foi possível cadastrar o atendimento.'], 400);
        }
    }
