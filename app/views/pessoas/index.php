...
<script>
    ...
    function abrirFormulario() {
        cardFormulario.classList.remove('d-none'); window.scrollTo({
    ...
    async function carregarPessoas() {
        try {
            ...
            tbody.innerHTML = dados.map(p => `<tr>
                ...
                <td class="text-end"><button class="btn btn-sm btn-outline-primary" onclick="editarPessoa(${Number(p.id)})">Editar</button> <button class="btn btn-sm btn-outline-danger" onclick="inativarPessoa(${Number(p.id)})">Inativar</button></td></tr>`).join('');
    ...
    ync function inativarPessoa(id) {
